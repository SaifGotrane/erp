...
Deno.serve(async (req) => {
  try {
    ...
    const { full_name, email, password, role, phone, depot_id, showroom_id, base_salary, children_count, is_household_head, cnss_number } = body;
    ...
    const { error: insertError } = await admin.from("employees").insert({
      ...
      is_household_head: is_household_head ?? false,
      cnss_number: cnss_number ?? null,
    });
