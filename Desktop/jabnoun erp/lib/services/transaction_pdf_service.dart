...
class TransactionPdfService {
  ...
  Future<List<int>> _build(Map<String, dynamic> d, Map<String, dynamic> c, String title, bool sale) async {
    ...
    pw.Widget info(String heading, Map x) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text(heading, style: pw.TextStyle(fontWeight: pw.FontWeight.bold)), pw.Text(text(x,'company_name').isNotEmpty ? text(x,'company_name') : text(x,'name')), if(text(x,'address').isNotEmpty) pw.Text(text(x,'address')), if(text(x,'phone').isNotEmpty || text(x,'email').isNotEmpty) pw.Text('${text(x,'phone')} ${text(x,'email')}'), if(text(x,'tax_id').isNotEmpty) pw.Text('MF: ${text(x,'tax_id')}')]);
    document.addPage(pw.MultiPage(pageFormat: PdfPageFormat.a4, margin: const pw.EdgeInsets.all(32), build: (_) => [
      ...
      pw.SizedBox(height: 18), pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [info(sale ? 'CLIENT' : 'FOURNISSEUR', partner), info(sale ? 'DÉPÔT / SHOWROOM' : 'DÉPÔT', location)]), pw.SizedBox(height: 18),
      ...
      pw.SizedBox(height: 16), pw.Align(alignment: pw.Alignment.centerRight, child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [pw.Text('Total HT: ${money.format(d['total_ht']??0)}'),pw.Text('TVA: ${money.format(d['total_tva']??0)}'),pw.Text('Total TTC: ${money.format(d['total_ttc']??0)}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),pw.Text('Payé: ${money.format(d['amount_paid']??0)}'), if(text(d,'payment_method').isNotEmpty) pw.Text('Paiement: ${text((d['payment_method'] as Map?)?.cast<String,dynamic>()??{},'name')}') ])), if(text(d,'notes').isNotEmpty) pw.Padding(padding: const pw.EdgeInsets.only(top: 16), child: pw.Text('Notes: ${text(d,'notes')}')),
