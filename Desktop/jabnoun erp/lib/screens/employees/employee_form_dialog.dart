...
class _EmployeeFormDialogState extends ConsumerState<_EmployeeFormDialog> {
  ...
  final _cnssNumber = TextEditingController();
  bool _saving = false;
  ...
  void initState() {
    ...
    _baseSalary.text = (e?.baseSalary ?? 0).toStringAsFixed(3);
    _childrenCount.text = (e?.childrenCount ?? 0).toString();
    _isHouseholdHead = e?.isHouseholdHead ?? false;
    _cnssNumber.text = e?.cnssNumber ?? '';
  }
    ...
    try {
      ...
      if (_isEdit) {
        await repo.updateEmployee(widget.employee!.id, {
          ...
          'base_salary': double.tryParse(_baseSalary.text.replaceAll(',', '.')) ?? 0,
          'children_count': int.tryParse(_childrenCount.text) ?? 0,
          'is_household_head': _isHouseholdHead,
          'cnss_number': _cnssNumber.text.trim().isEmpty ? null : _cnssNumber.text.trim(),
        });
      } else {
        await repo.createEmployee(
          ...
          baseSalary: double.tryParse(_baseSalary.text.replaceAll(',', '.')) ?? 0,
          childrenCount: int.tryParse(_childrenCount.text) ?? 0,
          isHouseholdHead: _isHouseholdHead,
          cnssNumber: _cnssNumber.text.trim().isEmpty ? null : _cnssNumber.text.trim(),
        );
      }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: TextFormField(
                        ...
                        decoration: const InputDecoration(labelText: 'Nombre d\'enfants à charge'),
                    ...
                    Expanded(
                      child: DropdownButtonFormField<bool>(
                        ...
                        onChanged: (v) => setState(() => _isHouseholdHead = v ?? false),
                      ),
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                TextFormField(
                  controller: _cnssNumber,
                  decoration: const InputDecoration(labelText: 'N° CNSS (optionnel)'),
                ),
                const SizedBox(height: 12),
                depotsAsync.when(
