...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  final _cnssEmployeeRate = TextEditingController();
  final _cssRate = TextEditingController();
  final _professionalExpensesRate = TextEditingController();
  final _professionalExpensesAnnualCap = TextEditingController();
  final _headOfHouseholdAnnualDeduction = TextEditingController();
  final _childAnnualDeduction = TextEditingController();
  final _maxDeductibleChildren = TextEditingController();
  ...
  void dispose() {
    ...
    _cnssEmployeeRate.dispose();
    _cssRate.dispose();
    _professionalExpensesRate.dispose();
    _professionalExpensesAnnualCap.dispose();
    _headOfHouseholdAnnualDeduction.dispose();
    _childAnnualDeduction.dispose();
    _maxDeductibleChildren.dispose();
    super.dispose();
  }

  double _parseRatePercent(TextEditingController c) => (double.tryParse(c.text.replaceAll(',', '.')) ?? 0) / 100;
  double _parseAmount(TextEditingController c) => double.tryParse(c.text.replaceAll(',', '.')) ?? 0;
  ...
  Future<void> _loadSettings() async {
    try {
      ...
      _cnssEmployeeRate.text = (settings.cnssEmployeeRate * 100).toStringAsFixed(2);
      _cssRate.text = (settings.cssRate * 100).toStringAsFixed(2);
      _professionalExpensesRate.text = (settings.professionalExpensesRate * 100).toStringAsFixed(2);
      _professionalExpensesAnnualCap.text = settings.professionalExpensesAnnualCap.toStringAsFixed(3);
      _headOfHouseholdAnnualDeduction.text = settings.headOfHouseholdAnnualDeduction.toStringAsFixed(3);
      _childAnnualDeduction.text = settings.childAnnualDeduction.toStringAsFixed(3);
      _maxDeductibleChildren.text = settings.maxDeductibleChildren.toString();
    } catch (_) {
    } finally {
      ...
      await ref
          ...
          .updateSettings(
            CompanySettings(
              ...
              cnssEmployeeRate: _parseRatePercent(_cnssEmployeeRate),
              cssRate: _parseRatePercent(_cssRate),
              professionalExpensesRate: _parseRatePercent(_professionalExpensesRate),
              professionalExpensesAnnualCap: _parseAmount(_professionalExpensesAnnualCap),
              headOfHouseholdAnnualDeduction: _parseAmount(_headOfHouseholdAnnualDeduction),
              childAnnualDeduction: _parseAmount(_childAnnualDeduction),
              maxDeductibleChildren: _parseInt(_maxDeductibleChildren),
            ),
          );
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                const Text(
                  'Taux et plafonds utilisés pour le calcul des fiches de paie (CNSS, CSS, abattements). À configurer une seule fois.',
                  style: TextStyle(fontSize: 12, color: AppColors.textMuted),
                ),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(
                    child: TextField(
                      controller: _cnssEmployeeRate,
                      decoration: const InputDecoration(labelText: 'Taux CNSS salarié (%)'),
                  ...
                  Expanded(
                    child: TextField(
                      controller: _cssRate,
                      decoration: const InputDecoration(labelText: 'Taux CSS (%)'),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    ),
                  ),
                ]),
                ...
                Row(children: [
                  Expanded(
                    child: TextField(
                      controller: _professionalExpensesRate,
                      decoration: const InputDecoration(labelText: 'Abattement frais professionnels (%)'),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: TextField(
                      controller: _professionalExpensesAnnualCap,
                      decoration: const InputDecoration(labelText: 'Plafond annuel abattement (TND)'),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    ),
                  ),
                ]),
                ...
                Row(children: [
                  Expanded(
                    child: TextField(
                      controller: _headOfHouseholdAnnualDeduction,
                      decoration: const InputDecoration(labelText: 'Déduction chef de famille (TND/an)'),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    ),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: TextField(
                      controller: _childAnnualDeduction,
                      decoration: const InputDecoration(labelText: 'Déduction par enfant (TND/an)'),
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                    ),
                  ),
                ]),
                const SizedBox(height: 12),
                TextField(
                  controller: _maxDeductibleChildren,
                  decoration: const InputDecoration(labelText: 'Nombre maximum d\'enfants déductibles'),
